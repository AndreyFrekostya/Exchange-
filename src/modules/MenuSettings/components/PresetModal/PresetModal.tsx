...
const PresetModal:React.FC<IModalPreset>= ({active, onClose}) => {
    ...
    const addPresetFunc=()=>{
        dispatch(addPreset({name: text, settings: graphics, mode: mode?.name}))
        setText('')
    }
  ...
  return (
    <div ref={ref} style={{overflowY: heightMain > 400 ? 'scroll' : 'auto'}} className={styles.main}>
          <div className={styles.wrap}>
            <div className={styles.inputField}>
              <input type="text" {...register('preset',{required:true})} value={text} placeholder='Сохранить шаблон' onChange={(e:React.ChangeEvent<HTMLInputElement>)=>setText(e.target.value)} />
              <CheckIcon onClick={handleSubmit(addPresetFunc)}/>
            </div>
            <div className={styles.error}>
              {errors?.preset && <p>Введите название!</p>}
            </div>
          </div>
